...
class MakeSeederCommand extends Command
{
  ...
  public function handle()
  {
    try {
      ...
      if (!$name) {
        throw new \Exception('Seeder table name required');
      }
      ...
      echo "Seeder created: " . $this->str->greenText($_params['path']);
  ...
  private function parseName($dbConn, $name)
  {
    ...
    $limit = $this->input->option('limit') ?? $this->config->limitSeed;

    $results = $this->fetchResults($db, $tableName, $limit);
